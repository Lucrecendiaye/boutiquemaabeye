  ...
  _startListeners() {
    ...
    this.COLS.forEach(col => {
      this._unsubs[col] = this.db
        .collection('users').doc(uid).collection(col)
        .onSnapshot(snap => {
          // Ne pas traiter si c'est nous-mêmes qui venons d'écrire
          // (évite le re-render inutile mais ne bloque pas la sync)
          const items = snap.docs.map(d => ({ id: d.id, ...d.data() }));
          DB.data[col] = items;
          localStorage.setItem(DB.key, JSON.stringify(DB.data));
          this._refreshCurrentPage();
          this._indicator(true);
        }, err => console.warn('[Cloud] listener ' + col + ':', err.message));
    });
    ...
    console.log('[Cloud] Listeners actifs');
  },
