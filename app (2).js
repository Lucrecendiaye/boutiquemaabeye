...
const Cloud = {
  ...
  async writeDoc(col, item) {
    ...
    console.log('[Cloud] ÉCRITURE → ' + col + '/' + docId);
    try {
      await this.db
        .collection('users').doc(uid).collection(col).doc(docId)
        .set(item);
      console.log('[Cloud] ÉCRITURE OK → ' + col + '/' + docId);
      this._indicator(true);
    } catch(err) {
      console.error('[Cloud] ÉCRITURE ERREUR ' + col + '/' + docId + ' :', err.message);
  ...
  set(key, val) {
    ...
    newArr.forEach(item => {
      ...
      if (!old || JSON.stringify(old) !== JSON.stringify(item)) {
        console.log('[Cloud] DB.set déclenche writeDoc → ' + key + '/' + item.id);
